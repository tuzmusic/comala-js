...
describe('Tag class', () => {
  ...
  it('can add a parameter later', () => {
    ...
    expect(tag.markup).toEqual(lines([
      '{tag:paramA=abc|paramB=abc|paramC=xyz}', '{tag}'
    ]));
  ...
  it('can reproduce our whole current workflow', () => {
    ...
    const labelTrigger = new Tag('trigger', { _: 'labeladded' });
    ...
    inProgress.addChild(
      new Tag('approval', { _: 'Review', assignable: 'true' })
    )
