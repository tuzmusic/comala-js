...
describe('real tag testing', () => {
  describe('expect', () => {
    ...
    test('known tag', () => {
      ...
      expect(chainer.stored).toEqual({ type: 'tag', content: { state: { name: 'state1' } } });
      source.expect({ approvalNamed: 'approval1' });
      expect(chainer.stored).toEqual({ type: 'tag', content: { approval: { name: 'approval1' } } });
      source.expect({ triggerNamed: 'trigger1' });
      expect(chainer.stored).toEqual({ type: 'tag', content: { trigger: { name: 'trigger1' } } });
    });
    
    test('unknown tag', () => {
      source.expect({ tagNamed: 'task' });
      expect(chainer.stored).toEqual({ type: 'tag', content: { task: { name: null } } });
    });
  ...
  test('params', () => {
    const source = '{state:Name1|param1=value1}' +
      '{state}' +
